...
def test_speed_and_count_round_trip(gripper: Robotiq2F85, speed: float) -> None:
...
def test_force_and_count_round_trip(gripper: Robotiq2F85, force: float) -> None:
    count = gripper.force_to_count(force)
    assert gripper.count_to_force(count) == pytest.approx(force, abs=1.0)


def test_speed_and_force_counts_stay_in_register_range(gripper: Robotiq2F85) -> None:
...
def test_current_conversion(gripper: Robotiq2F85) -> None:
    assert gripper.count_to_current(0) == pytest.approx(0.0)
    assert gripper.count_to_current(100) == pytest.approx(10.0)


def test_tcp_z_matches_the_closed_form_model(gripper: Robotiq2F85) -> None:
...
def test_tcp_z_is_maximal_where_the_fingers_are_vertical(gripper: Robotiq2F85) -> None:
